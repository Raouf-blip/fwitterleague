<template>
  <div>
    ...
    <BaseCard v-for="member in team" :key="member.name" :hoverable="false" class="!p-6 text-center">
      <div class="w-16 h-16 rounded-full bg-[#5865F2]/20 border border-[#5865F2]/30 flex items-center justify-center mx-auto mb-4">
        <svg class="w-7 h-7 text-[#5865F2]" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="mt-4 px-3 py-2 bg-white/5 rounded-lg border border-border">
        <span class="text-sm text-text-secondary font-mono">{{ member.discord }}</span>
      </div>
    </BaseCard>
    </div>
  </div>

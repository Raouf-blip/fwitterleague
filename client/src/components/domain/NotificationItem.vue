  ...
  >
    ...
    <div class="flex-1 min-w-0">
      ...
      <div v-if="item.message" class="mt-1">
        <p
          ...
          :class="{ 'line-clamp-2': !showFullMessage }"
        >
          {{ item.message }}
        </p>
        <button
      ...
      <div
        v-if="
          item.sender && item.type === 'application' && direction === 'inbox'
        "
      ...
      <div
        v-if="item.team && item.type === 'offer' && direction === 'inbox'"
        class="text-xs text-text-muted mt-1.5"
      >
        Équipe :
        <strong class="text-text-secondary"
          >{{ item.team.name }} [{{ item.team.tag }}]</strong
        >
      </div>
      <div
        v-if="
          item.team && item.type === 'application' && direction === 'outbox'
        "
        class="text-xs text-text-muted mt-1.5"
      >
      ...
      <div
        v-if="item.sender && item.type === 'offer' && direction === 'outbox'"
        class="text-xs text-text-muted mt-1.5"
      >
        Joueur invité :
        <strong class="text-text-secondary">{{ item.sender.username }}</strong>
    ...
    <div class="flex items-center gap-2 shrink-0">
      <template
        v-if="
          (item.status === 'pending' ||
            (item.type === 'scrim_request' && !item.is_read)) &&
          direction === 'inbox'
        "
      >
        <BaseButton variant="primary" size="sm" @click="$emit('accept', item)"
          >Accepter</BaseButton
        >
        <BaseButton variant="ghost" size="sm" @click="$emit('reject', item)"
          >Refuser</BaseButton
        >
      </template>
...
const isUnread = computed(() => {
  ...
  if (props.item.status === "pending" && props.direction === "inbox")
    return true;
  return false;
});
...
const statusLabel = computed(
  () => statusConfig.value?.label || props.item.status,
);
const statusVariant = computed(
  () =>
    (statusConfig.value?.color || "muted") as
      | "warning"
      | "success"
      | "danger"
      | "muted",

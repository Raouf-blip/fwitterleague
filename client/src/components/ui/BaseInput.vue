<template>
  <div class="flex flex-col gap-1.5">
    <label v-if="label" :for="id" class="text-sm font-bold text-text-secondary">
      {{ label }}
    ...
    <div class="relative">
      <input
        ...
        :class="{ 'pr-12': $slots.trailing }"
        @input="$emit('update:modelValue', ($event.target as HTMLInputElement).value)"
      />
      <div v-if="$slots.trailing" class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center">
        <slot name="trailing" />
      </div>
    </div>
